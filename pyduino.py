...
def _join_command(self, pin_number, joinString, headerString):
    command = (''.join((joinString, str(pin_number)))).encode()
    self.conn.write(command)
    line_received = self.conn.readline().decode().strip()
    header, value = line_received.split(':')  # e.g. A4:1
    if header == (headerString + str(pin_number)):
        return int(value)
...
class Arduino():
    ...
    def digital_read(self, pin_number):
        ...
        return _join_command(self, pin_number, 'RD', 'D')
    ...
    def digital_write(self, pin_number, digital_value):
        ...
        connection """
        command = (
            ''.join(('WD', str(pin_number), ':', str(digital_value)))).encode()
        self.conn.write(command)
    ...
    def analog_read(self, pin_number):
        ...
        return _join_command(self, pin_number, 'RA', 'A')
    ...
    def analog_write(self, pin_number, analog_value):
        ...
        connection """
        command = (
            ''.join(('WA', str(pin_number), ':', str(analog_value)))).encode()
        self.conn.write(command)
